import {Container, NineSliceSprite, Texture, Text} from 'pixi.js'
...
export default class LevelResultsReward extends Container {
  ...
  get difficultyData() {
    ...
    if (levelType === LEVEL_TYPES.WORDS.name)     return {levelType, textBonus: `${i18next.t('difficultyLevels.veryHard')}`, reward: REWARD.veryHard}
    if (levelType === LEVEL_TYPES.ANAGRAMS.name)  return {levelType, textBonus: `${i18next.t('difficultyLevels.veryHard')}`, reward: REWARD.veryHard}
    if (levelType === LEVEL_TYPES.GENERATOR.name) return {levelType, textBonus: `${i18next.t('difficultyLevels.extreme')}`, reward: REWARD.extreme}
  ...
  #hide = async (delay = 0) => {
    await gsap.timeline()
      .to(this, {alpha: 0, duration: 0.3, delay: delay, visible: false})
    this.destroy()
  ...
  #createRowBonus = () => {
    ...
    const textBonusName = GameUtils.createText(
      GameUtils.capitalize(textBonus),
      {style: this.#rowTextStyle, name: 'textBonusName', anchorX: 0}
    )
    textBonusName.x = -140
