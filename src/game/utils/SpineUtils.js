...
export default class SpineUtils {
  ...
  static getSlotView = (spine, slot) => {
    ...
    const worldVertices = new Float32Array(
      attachment instanceof RegionAttachment ? 8 : attachment.worldVerticesLength
    )
    ...
    const indices = attachment instanceof RegionAttachment
  ...
  static createSpine = (props = {}) => {
    ...
    try {
      ...
      const skin =
        spine.skeleton.data.findSkin(skinName) ??
        spine.skeleton.data.defaultSkin ??
        spine.skeleton.data.skins[0]
  ...
  static spineParser = (spines, exists = 'webp') => {
    try {
      [...spines].forEach(spine => {
        ...
        Assets.cache.set(
          `${name}.spineData`,
          spineJsonParser.readSkeletonData(json)
        )
      })
    } catch (e) {
