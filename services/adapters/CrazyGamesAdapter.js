...
class Adapter {
  ...
  outputInfo() {
    console.log("Adapter: " + "CrazyGames");
...
;// ./adapters/CrazyGamesBasic/cg.ts


;
const cgSdk = new AsyncObject(() => {
  return includeScript("https://sdk.crazygames.com/crazygames-sdk-v3.js").then(() => {
    return getCG()?.init() || Promise.reject("CrazyGames SDK not found");
  });
});
function initCG() {
  return cgSdk.get();
...
;
;// ./adapters/CrazyGamesBasic/CGStorage.ts
function CGStorage_defineProperty(e, r, t) { return (r = CGStorage_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function CGStorage_toPropertyKey(t) { var i = CGStorage_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function CGStorage_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }



class CGStorage extends Storage {
  constructor() {
    super(...arguments);
    CGStorage_defineProperty(this, "subLocalStorage", new SubLocalStorage());
  }
  getLocalStorage() {
    return this.subLocalStorage;
  }
  getFromPlatform(keys) {
    return Promise.resolve(null);
  }
  setToPlatform(data, force) {
    this.subLocalStorage.save();
    return Promise.resolve();
  }
}
;
const DELAY = SECOND * 30;
class SubLocalStorage {
  constructor() {
    CGStorage_defineProperty(this, "store", {});
    CGStorage_defineProperty(this, "timeout", null);
  }
  getData() {
    return getCG()?.data;
  }
  ...
  getItem(key) {
    if (!this.hasItem(key)) this.loadItem(key);
  ...
  loadItem(key) {
    this.store[key] = this.getData().getItem(key);
  }
  removeItem(key) {
    delete this.store[key];
    this.tryDataFunction("removeItem", key);
  }
  clear() {
    this.store = {};
    this.tryDataFunction("clear");
  }
  key(index) {
    return this.tryDataFunction("key", index) || Object.keys(this.store)[index];
  }
  get length() {
    return this.getData().length || Object.keys(this.store).length;
  }
  delaySave() {
  ...
  rewrite() {
    Object.keys(this.store).forEach(key => {
      this.getData().setItem(key, this.store[key]);
    });
  }
  tryDataFunction(funcName) {
    try {
      for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
        args[_key - 1] = arguments[_key];
      }
      return this.getData()[funcName](...args);
    } catch (e) {
      console.warn("tryDataFunction()", e);
      return null;
    }
  }
}
;
;// ./adapters/CrazyGamesBasic/CrazyGamesBasicAdapter.ts
function CrazyGamesBasicAdapter_defineProperty(e, r, t) { return (r = CrazyGamesBasicAdapter_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function CrazyGamesBasicAdapter_toPropertyKey(t) { var i = CrazyGamesBasicAdapter_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function CrazyGamesBasicAdapter_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }





class CrazyGamesBasicAdapter extends Adapter {
  constructor() {
    super(...arguments);
    CrazyGamesBasicAdapter_defineProperty(this, "platform_prefix", "cg");
    CrazyGamesBasicAdapter_defineProperty(this, "lastSettings", {});
  }
  initApi() {
    return initCG().then(() => {
      this.loadingStart();
  ...
  initEvents() {
    getCG()?.game.addSettingsChangeListener(newSettings => {
  ...
  updateSettings(newSettings) {
    Object.keys(newSettings).forEach(key => {
      ...
      if (this.lastSettings[key] === value) return;
      this.lastSettings[key] = value;
      const [eventName, ...args] = this.getEventNameAndArgumets(key, value);
      if (eventName) this.events.emit(eventName, ...args);
    });
  }
  getEventNameAndArgumets(key, value) {
    if (key === "muteAudio") {
      if (value) {
        return [AUDIO_OFF_EVENT];
      } else {
        return [AUDIO_ON_EVENT];
      }
    }
    return [];
  }
  ...
  happytime() {
    getCG()?.game.happytime();
  }
  gameplayStart() {
    getCG()?.game.gameplayStart();
  }
  gameplayStop() {
    getCG()?.game.gameplayStop();
  }
  gameReady() {
    this.loadingStop();
  }
  loadingStart() {
    getCG()?.game.loadingStart();
  }
  loadingStop() {
    getCG()?.game.loadingStop();
  }
  createStorage() {
    return new CGStorage(this);
  }
}
;
;// ./adapters/CrazyGames/CGAdvertising.ts
function CGAdvertising_defineProperty(e, r, t) { return (r = CGAdvertising_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function CGAdvertising_toPropertyKey(t) { var i = CGAdvertising_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function CGAdvertising_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }




const INTERSTITIAL_TYPE_NAME = "midgame";
const REWARDED_TYPE_NAME = "rewarded";
class CGAdvertising extends Advertising {
  constructor() {
    super(...arguments);
    CGAdvertising_defineProperty(this, "adblock", false);
    CGAdvertising_defineProperty(this, "adaptiveBanners", new CGBannersCore());
  }
  async init() {
    this.adblock = !!(await getCG()?.ad?.hasAdblock());
    return super.init();
  }
  isRewardedAvailableNow() {
    return !this.adblock;
  ...
  showInterstitial() {
    return this.requestAd(INTERSTITIAL_TYPE_NAME);
  }
  showRewarded() {
    return this.requestAd(REWARDED_TYPE_NAME);
  }
  requestAd() {
    let type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : INTERSTITIAL_TYPE_NAME;
    const {
        promise,
        resolve,
        reject
      ...
      callbacks = {
        adFinished: () => {
          console.log("End " + type + " ad");
          resolve();
        },
        adError: error => {
          console.log("Error " + type + " ad", error);
          reject(error);
        },
        adStarted: () => console.log("Start " + type + " ad")
      };
    getCG()?.ad?.requestAd(type, callbacks);
    return promise;
  }
  showPreroll() {
    return Promise.reject();
  ...
  setAdaptiveBannersAreas(areas) {
    this.adaptiveBanners.setAreas(areas);
  }
  showAllAdaptiveBanners() {
    this.adaptiveBanners.show();
  }
  hideAllAdaptiveBanners() {
    this.adaptiveBanners.hide();
  }
  setAdaptiveBannersVisibility(array) {
    this.adaptiveBanners.setVisibility(array);
  }
  debugAdaptiveBanners() {
    this.adaptiveBanners.debug();
  }
  preloadAdaptiveBanners(mask) {
    this.adaptiveBanners.preload(mask);
  }
}
;
class CGBannersCore {
  constructor() {
    CGAdvertising_defineProperty(this, "banners", []);
    CGAdvertising_defineProperty(this, "_debug", false);
  }
  debug() {
    this._debug = true;
    this.banners.forEach(banner => banner.debug());
  }
  show() {
    this.banners.forEach(banner => banner.show());
  }
  hide() {
    this.banners.forEach(banner => banner.hide());
  }
  preload(mask) {
  ...
  setAreas(areas) {
    const count = Math.max(areas.length, this.banners.length);
    for (let i = 0; i < count; i++) {
      const banner = this.banners[i] || this.addBanner();
      banner.setArea(areas[i] || {
        x: 0,
        y: 0,
        width: 0,
        height: 0
  ...
  addBanner() {
    const banner = new CGBanner();
    if (this._debug) banner.debug();
...
const BANNER_ID_PREFIX = "adaptive_banner_";
...
function getNewBannerId() {
  return BANNER_ID_PREFIX + ++bannersCounter;
}
class CGBanner {
  constructor() {
    CGAdvertising_defineProperty(this, "id", "");
    CGAdvertising_defineProperty(this, "banner", null);
    CGAdvertising_defineProperty(this, "visible", false);
    CGAdvertising_defineProperty(this, "lastUpdate", 0);
    CGAdvertising_defineProperty(this, "actualArea", {
      x: 0,
      y: 0,
      width: 0,
      height: 0
    });
    CGAdvertising_defineProperty(this, "preloaded", false);
    CGAdvertising_defineProperty(this, "timeout", null);
    this.createBanner();
  }
  debug() {
    this.banner.style.border = "2px solid #ffffff";
    this.banner.style.backgroundColor = "rgba(225, 150, 29, 0.5)";
  }
  isValidArea() {
    return BANNERS_SIZES.find(size => size.w <= this.actualArea.width && size.h <= this.actualArea.height) !== undefined;
  }
  setArea(area) {
    const wasValid = this.isValidArea();
    this.actualArea = area;
    if (!this.isValidArea()) {
      this.banner.style.display = "none";
      return;
    }
    this.banner.style.top = area.y + "px";
    this.banner.style.left = area.x + "px";
    this.banner.style.width = area.width + "px";
    this.banner.style.height = area.height + "px";
  ...
  tryUpdate() {
    ...
    if (!this.isValidArea() || this.isNewContent()) return;
    if (needClear) this.clear();
    this.update().catch(() => {}).finally(() => {
      if (this.visible) this.startIntervalUpdate();
    });
  }
  update() {
    this.lastUpdate = Date.now();
    return this.requestResponsiveBanner();
  }
  isNewContent() {
    return Date.now() - this.lastUpdate < UPDATE_INTERVAL;
  }
  async requestResponsiveBanner() {
    try {
      await getCG()?.banner?.requestResponsiveBanner(this.id);
      this.lastUpdate = Date.now();
    } catch (e) {
  ...
  clear() {
    getCG()?.banner?.clearBanner(this.id);
  }
  startIntervalUpdate() {
    this.stopIntervalUpdate();
    this.timeout = setTimeout(() => {
...
;
;// ./adapters/CrazyGames/CrazyGamesAdapter.ts


class CrazyGamesAdapter extends CrazyGamesBasicAdapter {
  createAdvertising() {
    return new CGAdvertising(this);
  }
}
;
