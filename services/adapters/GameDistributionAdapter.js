...
__webpack_require__.d(__webpack_exports__, {
  A: () => (/* binding */ GameDistributionAdapter)
});
...
class Adapter {
  ...
  outputInfo() {
    console.log("Adapter: " + "GameDistribution");
...
;// ./adapters/GameDistribution/gdConstants.ts
const GD_SDK_URL = "https://html5.api.gamedistribution.com/main.min.js";
const GD_SDK_TAG_ID = "gamedistribution-jssdk";
const SDK_READY = "SDK_READY";
const SDK_ERROR = "SDK_ERROR";
const SDK_GAME_PAUSE = "SDK_GAME_PAUSE";
const SDK_GAME_START = "SDK_GAME_START";
;// ./adapters/GameDistribution/gdSdk.ts





const gdsdk = new AsyncObject(initGameDistribution);
let isReady = false;
const events = new EventsCore();
function initGdsdk(gameId) {
  return gdsdk.get(gameId);
}
function getGdsdk() {
  return gdsdk.get();
}
function initGameDistribution(gameId) {
  console.log("initGameDistribution", gameId);
  initOptions(gameId);
  return includeScript(GD_SDK_URL, GD_SDK_TAG_ID).then(() => {
    return waitGdSdk();
  }).catch(e => {
    console.error("Error initializing game distribution", e);
    return Promise.reject(e);
  });
}
;
function waitGdSdk() {
  if (isReady) return Promise.resolve(window["gdsdk"]);
  return new Promise((resolve, reject) => {
    events.once(SDK_READY, () => resolve(window["gdsdk"]));
  });
}
function initOptions(gameId) {
  window["GD_OPTIONS"] = {
    gameId,
    onEvent: function (event) {
      switch (event.name) {
        case SDK_READY:
          isReady = true;
          events.emit(SDK_READY);
          break;
        case SDK_ERROR:
          events.emit(SDK_ERROR);
          break;
        case SDK_GAME_PAUSE:
          events.emit(PAUSE_EVENT);
          break;
        case SDK_GAME_START:
          events.emit(RESUME_EVENT);
          break;
        default:
          console.log("GD unhandled event", event.name);
          break;
      }
    }
  };
...
;
;// ./adapters/GameDistribution/GdAds.ts
function GdAds_defineProperty(e, r, t) { return (r = GdAds_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function GdAds_toPropertyKey(t) { var i = GdAds_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function GdAds_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }
...
class GdAds {
  constructor() {
    ...
    this.type = type;
    GdAds_defineProperty(this, "ready", false);
    GdAds_defineProperty(this, "loading", false);
    GdAds_defineProperty(this, "result", false);
    events.once(SDK_READY, () => this.update());
  }
  complete() {
    this.result = true;
  }
  preload() {
    ...
    this.loading = true;
    getGdsdk().then(sdk => sdk.preloadAd(this.type)).then(e => {
      this.ready = true;
      console.log("preload ads success", e);
    }).catch(e => {
      this.ready = false;
      console.error("preload ads failed", e);
    }).finally(() => this.loading = false);
  }
  update() {
    if (!this.type) this.ready = true;
    if (this.ready) return;
    this.preload();
  }
  show() {
    var _this = this;
    console.log("call show ad", this.type);
    if (!this.ready) {
      this.update();
      return Promise.reject(new Error("Ads not ready"));
    }
    this.result = false;
    return getGdsdk().then(sdk => sdk.showAd(this.type || undefined)).then(e => {
      console.log("showing ads result", e, this.type);
      const success = this.type === "rewarded" ? !!e?.args?.success : true;
      if (success) {
        console.log("showing ads success", e);
        return Promise.resolve();
      } else {
        return Promise.reject(e);
      }
    }).catch(e => {
      console.error("showing ads failed", e);
      return Promise.reject(e);
    }).finally(function () {
      for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
        args[_key] = arguments[_key];
      }
      console.log("showing ads finally", args);
      _this.ready = false;
      _this.update();
    });
  }
...
;
;// ./adapters/GameDistribution/GdAdvertising.ts
function GdAdvertising_defineProperty(e, r, t) { return (r = GdAdvertising_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function GdAdvertising_toPropertyKey(t) { var i = GdAdvertising_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function GdAdvertising_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


class GdAdvertising extends Advertising {
  constructor() {
    super(...arguments);
    GdAdvertising_defineProperty(this, "rules", {
      preroll: true,
      pause: true,
      tutorial: true
    });
    GdAdvertising_defineProperty(this, "interstitial", new GdAds(""));
    GdAdvertising_defineProperty(this, "rewarded", new GdAds("rewarded"));
  }
  _showInterstitial() {
...
;// ./adapters/GameDistribution/GameDistributionAdapter.ts
...
class GameDistributionAdapter extends Adapter {
  constructor(options) {
    super(options);
    this.events = events;
  }
  ...
  initApi() {
    return initGdsdk(this.options.app_id || "");
  }
  createServer() {
    return new DisabledServer(this);
  }
  createAdvertising() {
    return new GdAdvertising(this);
  }
  createLeaderboard() {
    return new EmptyLeaderboard(this);
  }
