...
;// ./adapters/FRVR/frvr.ts

const asyncInitFRVR = new AsyncObject_AsyncObject(() => {
  if (!getFRVR()) return Promise.reject(new Error("FRVR SDK not found"));
  console.log("FRVR", getFRVR());
  return getFRVR()?.init("prod").then(() => {
    console.log("FRVR.bootstrapper", getFRVR()?.bootstrapper);
    return getFRVR()?.bootstrapper.init();
  });
});

/*
includeScript("https://cdn.frvr.com/sdk/frvr-sdk.min.js")
includeScript("https://cdn.frvr.com/sdk/frvr-channel-dev.min.js")
*/

/*
getFRVR().config = { 
    gameId: "awesomegame", 
    cloudStorage: { playerStorage: { gameId: "G48WRPH" } }, 
    social: { syncFriendsOnLogin: false }, 
    iap: { catalog: { chestofgems: { label: "chestofgems", storeId: "chestofgems", consumable: true } } }, 
    ads: { autoInit: true, throttling: { maxfrequency: 30000 }, providers: [ ] }, 
    auth: { enableAnonymousLogin: true, facebookSdk: { permissions: "gaming_profile,gaming_user_picture" } }, 
    features: { gameId: "G48WRPH" }, 
    shop: { gameId: "G48WRPH" }, 
    consent: { providerName: "cookiepro", config: { websiteKey: "acd0a7d3-539e-483c-8c17-484beca00b4d" } } 
};
//*/
...
;
function initFRVR() {
  return asyncInitFRVR.get();
}
;
;// ./adapters/FRVR/FRVRAdvertising.ts
function FRVRAdvertising_defineProperty(e, r, t) { return (r = FRVRAdvertising_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function FRVRAdvertising_toPropertyKey(t) { var i = FRVRAdvertising_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function FRVRAdvertising_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }



class FRVRAdvertising extends Advertising {
  constructor() {
    super(...arguments);
    FRVRAdvertising_defineProperty(this, "interstitialInterval", MINUTE * 2);
  }
  showPreroll() {
    return Promise.resolve();
  }
  _showInterstitial() {
    return this.safeShow(getFRVR()?.ads.AdType.INTERSTITIAL);
  }
  isRewardedAvailableNow() {
    return !!getFRVR()?.ads.isReady(getFRVR()?.ads.AdType.REWARD);
  }
  showRewarded() {
    return this.safeShow(getFRVR()?.ads.AdType.REWARD).then(result => {
      if (result === getFRVR()?.ads.AdShowResult.COMPLETED) {
        return Promise.resolve();
      } else {
        return Promise.reject(new Error("Rewarded ad not completed"));
      }
    });
  }
  showBanner() {
    return this.safeShow(getFRVR()?.ads.AdType.BANNER);
  }
  hideBanner() {
    return this.safeCall("hide", getFRVR()?.ads.AdType.BANNER);
  }
  safeShow(type) {
    return this.safeCall("show", type);
  }
  safeCall(fn) {
    for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }
    return getFRVR()?.ads[fn](...args) || Promise.reject(new Error("FRVR not found"));
  }
}
;
;// ./adapters/FRVR/FRVRInvite.ts
function FRVRInvite_defineProperty(e, r, t) { return (r = FRVRInvite_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function FRVRInvite_toPropertyKey(t) { var i = FRVRInvite_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function FRVRInvite_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


class FRVRInvite extends SubModule {
  constructor() {
    super(...arguments);
    FRVRInvite_defineProperty(this, "available", true);
  }
  act() {
    let text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "invite";
    return getFRVR()?.social.platform.invite({
      text
    });
  }
}
;
;// ./adapters/FRVR/FRVRPlayer.ts
function FRVRPlayer_defineProperty(e, r, t) { return (r = FRVRPlayer_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function FRVRPlayer_toPropertyKey(t) { var i = FRVRPlayer_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function FRVRPlayer_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


;
class FRVRPlayer extends Player {
  constructor() {
    super(...arguments);
    FRVRPlayer_defineProperty(this, "playerData", null);
  }
  init() {
    return initFRVR().then(() => this.initPlayerData());
  }
  isAuthAvailable() {
    return true;
  }
  isAuth() {
    console.log("isAuth", getFRVR()?.auth, getFRVR()?.auth.getLoginStatus);
    return !!getFRVR()?.auth.isLoggedIn() || false;
  }
  auth() {
    return getFRVR()?.auth.login().then(() => this.initPlayerData());
  }
  async initPlayerData() {
    const frvr = getFRVR();
    if (!frvr) return Promise.reject(new Error("FRVR not found"));
    this.playerData = {
      id: await frvr.profile.id(),
      name: await frvr.profile.name(),
      avatar: await frvr.profile.image()
    };
  }
  getId() {
  ...
  getAvatar() {
    return this.playerData?.avatar || "";
  }
}
;
;// ./adapters/FRVR/FRVRShortcut.ts


class FRVRShortcut extends SubModule {
  init() {
    return getFRVR()?.shortcut.canCreateShortcut().then(isAvailable => {
  ...
  act(options) {
    return getFRVR()?.shortcut.createShortcut().then(() => {
      this.status = true;
    });
  }
}
;// ./adapters/FRVR/FRVRStorage.ts
function FRVRStorage_defineProperty(e, r, t) { return (r = FRVRStorage_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function FRVRStorage_toPropertyKey(t) { var i = FRVRStorage_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function FRVRStorage_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


;
class FRVRStorage extends Storage {
  constructor() {
    super(...arguments);
    FRVRStorage_defineProperty(this, "cloudStorage", new SubStorage("cloudStorage"));
    FRVRStorage_defineProperty(this, "localStorage", new SubStorage("localStorage"));
  }
  get(keys) {
    return Promise.all([this.cloudStorage.get(keys), this.localStorage.get(keys)]).then(results => {
      console.log("FRVRStorage get", results);
      return results.filter(item => !!item);
    });
  }
  set(data) {
    let force = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    console.log("FRVRStorage set", data);
    return Promise.all([this.cloudStorage.set(data, force), this.localStorage.set(data, force)]);
  }
}
;
class SubStorage {
  ...
  async get(keys) {
    if (!this.isWorking()) return Promise.resolve(null);
    const items = await this.getStorage().getItems(keys);
    if (!items || !Object.keys(items).length) return Promise.resolve(null);
    return items;
  }
  set(data) {
    let force = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (!this.isWorking()) return Promise.resolve(null);
    return this.getStorage().setItems(objectToArray(data)) || Promise.resolve(null);
  }
  isWorking() {
    return getFRVR()?.[this.name] !== undefined;
  }
  getStorage() {
    return getFRVR()?.[this.name];
  }
}
;
...
class FRVRAdapter extends Adapter {
  initApi() {
    return initFRVR();
  }
  initEvents() {
    const frvr = getFRVR();
    if (!frvr) return;
    frvr.lifecycle.onGamePause = () => {
      this.events.emit(PAUSE_EVENT);
    };
    frvr.lifecycle.onSuspend = () => {
      this.events.emit(PAUSE_EVENT);
    };
    frvr.lifecycle.onResume = () => {
      this.events.emit(RESUME_EVENT);
    };
    frvr.lifecycle.onAudioSuspend = () => {
      this.events.emit(AUDIO_OFF_EVENT);
    };
    frvr.lifecycle.onAudioResume = () => {
      this.events.emit(AUDIO_ON_EVENT);
    };
  }
  loading(v) {
    getFRVR()?.bootstrapper?.setProgress(v || 1);
  }
  gameReady() {
    getFRVR()?.bootstrapper?.setProgress(1);
    getFRVR()?.bootstrapper?.complete();
  }
  createServer() {
    return new DisabledServer(this);
  }
  createPlayer() {
    return new FRVRPlayer(this);
  }
  createStorage() {
    return new FRVRStorage(this);
  }
  createAdvertising() {
    return new FRVRAdvertising(this);
  }
  createLeaderboard() {
    return new EmptyLeaderboard(this);
  }
  ...
  createShortcut() {
    return new FRVRShortcut(this);
  }
  createInviteFriends() {
    return new FRVRInvite(this);
  }
}
;
