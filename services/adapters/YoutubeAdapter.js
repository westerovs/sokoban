...
;
function getYoutubeSdk() {
  return window.ytgame;
}
;
let lang = "en";
function initLang() {
  return getYoutubeSdk().system.getLanguage().then(localeTag => {
    lang = localeTag.split("-")[0];
  });
}
;
function getLang() {
...
;
;// ./adapters/Youtube/YoutubeAdvertising.ts


const REWARDED_AD_ID = "21403813-2e22-4316-a8b2-7d4f52a6f6fb";
class YoutubeAdvertising extends Advertising {
  showPreroll() {
    return Promise.resolve();
  }
  async _showInterstitial() {
    try {
      await getYoutubeSdk().ads.requestInterstitialAd();
      return Promise.resolve();
    } catch (error) {
      console.error("YoutubeAdvertising _showInterstitial error", error);
  ...
  async showRewarded() {
    try {
      const isRewardEarned = await getYoutubeSdk().ads.requestRewardedAd(REWARDED_AD_ID);
      if (!isRewardEarned) return Promise.reject("no reward earned");
      return Promise.resolve();
    } catch (error) {
      console.error("YoutubeAdvertising showRewarded error", error);
      return Promise.reject(error);
    }
  }
}
;
;// ./adapters/Youtube/YoutubeSetRecord.ts
function YoutubeSetRecord_defineProperty(e, r, t) { return (r = YoutubeSetRecord_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function YoutubeSetRecord_toPropertyKey(t) { var i = YoutubeSetRecord_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function YoutubeSetRecord_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }


class YoutubeSetRecord extends SubModule {
  constructor() {
    super(...arguments);
    YoutubeSetRecord_defineProperty(this, "available", true);
  }
  act(options) {
    try {
      const value = Math.floor(Number(options)) || 0;
      if (value <= 0) return Promise.reject("YoutubeSetRecord act: value must be greater than 0");
      return getYoutubeSdk().engagement.sendScore({
        value
      });
    } catch (error) {
      console.error("YoutubeSetRecord act: error", error);
      return Promise.reject(error);
    }
  }
}
;
;// ./adapters/Youtube/YoutubeStorage.ts



class YoutubeStorage extends Storage {
  async setToPlatform(data, force) {
    try {
      getYoutubeSdk().game.saveData(stringifyJSON(data));
      return Promise.resolve();
    } catch (error) {
      console.error("YoutubeStorage setToPlatform error", error);
      return Promise.reject(error);
    }
  }
  async getFromPlatform(keys) {
    try {
      const data = await getYoutubeSdk().game.loadData();
      return parseJSON(data, null);
    } catch (error) {
      console.error("YoutubeStorage getFromPlatform error", error);
      return null;
    }
  }
}
;
;// ./adapters/Youtube/YoutubeAdapter.ts
function YoutubeAdapter_defineProperty(e, r, t) { return (r = YoutubeAdapter_toPropertyKey(r)) in e ? Object.defineProperty(e, r, { value: t, enumerable: !0, configurable: !0, writable: !0 }) : e[r] = t, e; }
function YoutubeAdapter_toPropertyKey(t) { var i = YoutubeAdapter_toPrimitive(t, "string"); return "symbol" == typeof i ? i : i + ""; }
function YoutubeAdapter_toPrimitive(t, r) { if ("object" != typeof t || !t) return t; var e = t[Symbol.toPrimitive]; if (void 0 !== e) { var i = e.call(t, r || "default"); if ("object" != typeof i) return i; throw new TypeError("@@toPrimitive must return a primitive value."); } return ("string" === r ? String : Number)(t); }
...
class YoutubeAdapter extends Adapter {
  constructor() {
    super(...arguments);
    YoutubeAdapter_defineProperty(this, "platform_prefix", "youtube");
    YoutubeAdapter_defineProperty(this, "_isSoundEnabled", true);
  }
  initApi() {
    return initYoutubeSdk().then(() => {
      return initLang();
    }).then(() => {
      getYoutubeSdk().game.firstFrameReady();
    });
  }
  isSoundEnabled() {
    return this._isSoundEnabled && !this.isPaused();
  }
  domainlock(url) {}
  getLang() {
    return getLang();
  ...
  gameReady() {
    getYoutubeSdk().game.gameReady();
  }
  initEvents() {
    console.log("initEvents");
    this._isSoundEnabled = getYoutubeSdk().system.isAudioEnabled();
    getYoutubeSdk().system.onPause(() => {
      console.log("onPause");
      this.events.emit(PAUSE_EVENT);
    });
    getYoutubeSdk().system.onResume(() => {
      console.log("onResume");
      this.events.emit(RESUME_EVENT);
    });
    getYoutubeSdk().system.onAudioEnabledChange(isEnabled => {
      console.log("onAudioEnabledChange", isEnabled);
      this._isSoundEnabled = isEnabled;
      if (isEnabled) {
        this.events.emit(AUDIO_ON_EVENT);
      } else {
        this.events.emit(AUDIO_OFF_EVENT);
  ...
  createSetRecord() {
    return new YoutubeSetRecord(this);
  }
  createServer() {
    return new DisabledServer(this);
  }
  createStorage() {
    return new YoutubeStorage(this);
  }
  createAdvertising() {
    return new YoutubeAdvertising(this);
  }
}
;
