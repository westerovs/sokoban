...
const ALLOWED_SYMBOLS = new Set(['_', '#', ' ', '.', '$', '@', '-', '*']) // Допустимые обозначения клеток карты
const DIRECTIONS = Object.freeze([
  {x: 0, y: -1},
  {x: 1, y: 0},
  {x: 0, y: 1},
  {x: -1, y: 0},
]) // Направления обхода соседних клеток
...
const getPositions = (map: string[], predicate: (symbol: string, position: Position) => boolean): Position[] => {
...
const createIssue = (type: ValidationIssue['type'], message: string, positions: Position[] = []): ValidationIssue => ({
  type,
  message,
  positions,
})
...
const validateDimensions = (map: string[]): ValidationIssue[] => {
...
const validateSymbols = (map: string[]): ValidationIssue[] => {
...
const validateEntities = (map: string[]): ValidationIssue[] => {
  const issues: ValidationIssue[] = []
...
const isPassable = (map: string[], position: Position) => {
...
const getReachablePositions = (map: string[], start?: Position): Set<string> => {
...
const validateConnectivity = (map: string[]): ValidationIssue[] => {
...
const isBlocked = (map: string[], position: Position) => {
  const symbol = map[position.y]?.[position.x]
  return !symbol || symbol === '_' || symbol === '#'
}

// Проверяет условие, описанное операцией `isStaticCorner`.
const isStaticCorner = (map: string[], position: Position) => {
...
const validateBoxCorners = (map: string[]): ValidationIssue[] => {
...
const validateLevelMap = (map: string[]): ValidationResult => {
