...
const getCenterOffset = (map, width, height) => {
  return {
...
const placeMap = (targetMap, sourceMap, offset) => {
...
const shiftPositionKey = (positionKey, offset) => {
  const [x, y] = positionKey.split(':').map(Number)
  return `${x + offset.x}:${y + offset.y}`
}

// Переносит координаты всех текстур одного визуального слоя.
const shiftRoleAppearance = (roleAppearance, offset) => {
...
const shiftAppearance = (appearance, offset) => {
...
const expandEditorState = (level, appearance, width, height) => {
...
const includePosition = (bounds, x, y) => ({
...
const isInsideBounds = (positionKey, bounds) => {
  const [x, y] = positionKey.split(':').map(Number)
...
const cropRoleAppearance = (roleAppearance, bounds) => {
...
const cropAppearance = (appearance, bounds) => {
  if (!bounds) return {}
  const entries = Object.entries(appearance).map(([role, values]) => [role, cropRoleAppearance(values, bounds)])
...
const compactEditorState = (state) => {
